...
var m P c c_f P P_f e W R k d n l gy_obs gp_obs y dA;
varexo e_a e_m;
...
c_f = c(1);
P_f = P(1);
-P/(c(1)*P(1)*m)+bet*P(1)*(alp*exp(-alp*(gam+log(e(1))))*k^(alp-1)*n(1)^(1-alp)+(1-del)*exp(-(gam+log(e(1)))))/(c_f(1)*P_f(1)*m(1))=0;
W = l/n;
-(psi/(1-psi))*(c*P/(1-n))+l/n;
